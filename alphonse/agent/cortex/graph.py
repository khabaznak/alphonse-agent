...
from alphonse.agent.cortex.nodes import AbilityStateDiscoveryDeps
from alphonse.agent.cortex.nodes import DiscoveryLoopDeps
from alphonse.agent.cortex.nodes import DispatchDiscoveryDeps
from alphonse.agent.cortex.nodes import EmptyDiscoveryResultDeps
from alphonse.agent.cortex.nodes import FreshPlanningDeps
from alphonse.agent.cortex.nodes import PendingDiscoveryDeps
from alphonse.agent.cortex.nodes import PlanningDiscoveryDeps
from alphonse.agent.cortex.nodes import apology_node
from alphonse.agent.cortex.nodes import ask_question_node
from alphonse.agent.cortex.nodes import bind_answer_to_steps
from alphonse.agent.cortex.nodes import build_discovery_loop_state
...
from alphonse.agent.cortex.nodes import dispatch_discovery_result
...
from alphonse.agent.cortex.nodes import handle_ability_state_for_discovery
from alphonse.agent.cortex.nodes import handle_pending_interaction_for_discovery
from alphonse.agent.cortex.nodes import ingest_node
from alphonse.agent.cortex.nodes import is_effectively_empty_discovery_result
from alphonse.agent.cortex.nodes import plan_node
from alphonse.agent.cortex.nodes import run_capability_gap_tool
from alphonse.agent.cortex.nodes import run_discovery_loop_step
from alphonse.agent.cortex.nodes import run_fresh_discovery_for_message
from alphonse.agent.cortex.nodes import run_intent_discovery
from alphonse.agent.cortex.nodes import select_next_step_node
...
class CortexGraph:
    ...
    def build(self) -> StateGraph:
        ...
        graph.add_node("plan_node", self._plan_node_impl)
        graph.add_node("select_next_step_node", self._select_next_step_node_impl)
        graph.add_node("execute_tool_node", self._execute_tool_node_impl_stateful)
        graph.add_node("ask_question_node", self._ask_question_node_impl)
        graph.add_node("apology_node", self._apology_node_impl)
        graph.add_node("respond_node", self._respond_node_impl_stateful)
    ...
    def _plan_node_impl(self, state: CortexState) -> dict[str, Any]:
        return plan_node(
            state,
            run_intent_discovery=lambda s: self._run_intent_discovery(
                s,
                self._llm_client_from_state(s),
            ),
        )

    def _select_next_step_node_impl(self, state: CortexState) -> dict[str, Any]:
    ...
    def _execute_tool_node_impl_stateful(self, state: CortexState) -> dict[str, Any]:
        return execute_tool_node_stateful(
            state,
            llm_client_from_state=self._llm_client_from_state,
            execute_tool_node_impl=execute_tool_node,
            run_discovery_loop_step_with_llm=lambda s, loop_state, llm_client: run_discovery_loop_step(
                s,
                loop_state,
                llm_client,
    ...
    def _ask_question_node_impl(self, state: CortexState) -> dict[str, Any]:
    ...
    def _apology_node_impl(self, state: CortexState) -> dict[str, Any]:
        return apology_node(
            state,
            ...
            llm_client=self._llm_client_from_state(state),
        )

    def _respond_node_impl_stateful(self, state: CortexState) -> dict[str, Any]:
    ...
    def _run_intent_discovery(
        ...
        return run_intent_discovery(
            state,
            llm_client,
            deps=PlanningDiscoveryDeps(
                require_last_user_message=_require_last_user_message,
                handle_pending_interaction_for_discovery=lambda **kwargs: self._handle_pending_interaction_for_discovery(
                    **kwargs
                ),
                handle_ability_state_for_discovery=lambda **kwargs: self._handle_ability_state_for_discovery(
                    **kwargs
                ),
                run_fresh_discovery_for_message=lambda **kwargs: self._run_fresh_discovery_for_message(
                    **kwargs
                ),
            ),
        )
    ...
    def _run_fresh_discovery_for_message(
        ...
        *,
        ...
        return run_fresh_discovery_for_message(
            state=state,
            llm_client=llm_client,
            text=text,
            deps=FreshPlanningDeps(
                ...
                dispatch_discovery_result=lambda **kwargs: self._dispatch_discovery_result(
                    **kwargs
                ),
            ),
        )
    ...
    def _dispatch_discovery_result(
        ...
        *,
        ...
        return dispatch_discovery_result(
            state=state,
            llm_client=llm_client,
            ...
            deps=DispatchDiscoveryDeps(
                run_capability_gap_tool=run_capability_gap_tool,
                build_discovery_loop_state=build_discovery_loop_state,
            ),
        )
    ...
    def _handle_pending_interaction_for_discovery(
        ...
        *,
        ...
        return handle_pending_interaction_for_discovery(
            state=state,
            llm_client=llm_client,
            text=text,
            deps=PendingDiscoveryDeps(
                now_iso_utc=lambda: datetime.now(timezone.utc).isoformat(),
                is_effectively_empty_discovery_result=lambda result: is_effectively_empty_discovery_result(
                    result,
    ...
    def _handle_ability_state_for_discovery(
        ...
        *,
        ...
        return handle_ability_state_for_discovery(
            state=state,
            llm_client=llm_client,
            text=text,
