...
def execute_tool_node_stateful(
    ...
    *,
    ...
    run_discovery_loop_step_with_llm: Callable[[dict[str, Any], dict[str, Any], Any], dict[str, Any]],
) -> dict[str, Any]:
    llm_client = llm_client_from_state(state)
    return execute_tool_node_impl(
        state,
        run_discovery_loop_step=lambda s, loop_state: run_discovery_loop_step_with_llm(
            s,
            loop_state,
            llm_client,
        ),
