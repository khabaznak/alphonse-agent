...
class HandleTimedSignalsAction(Action):
    ...
    def execute(self, context: dict) -> ActionResult:
        ...
        logger.info(
            "HandleTimedSignalsAction invoked signal_id=%s timed_signal_id=%s correlation_id=%s",
            getattr(signal, "id", None),
            payload.get("timed_signal_id"),
            getattr(signal, "correlation_id", None),
        )
        if _looks_like_job_trigger(payload=payload, inner=inner):
        ...
        logger.info(
            "HandleTimedSignalsAction render message_len=%s channels=%s",
            len(rendered),
            ",".join(channels),
        )
        ...
        if target:
            ...
            logger.warning("HandleTimedSignalsAction missing target; skipping dispatch")
    ...
    def _dispatch_conscious_timed_signal(self, *, context: dict, payload: dict[str, Any], signal: Any) -> None:
        ...
        if not hasattr(bus, "emit"):
            logger.warning("HandleTimedSignalsAction conscious dispatch skipped reason=no_bus")
            return
        inner = _payload_from_signal(payload)
        message_text = _extract_prompt_text(inner=inner)
    ...
    def _handle_job_trigger(self, *, context: dict, payload: dict[str, Any], signal: Any) -> None:
        ...
        if not job_id or not user_id:
            logger.warning("HandleTimedSignalsAction missing job trigger ids job_id=%s user_id=%s", job_id, user_id)
            return
        bus = context.get("ctx")
        ...
        try:
            ...
            logger.info("HandleTimedSignalsAction job_trigger executed job_id=%s user_id=%s", job_id, user_id)
...
def _extract_prompt_text(*, inner: dict[str, Any]) -> str:
    text = str(
        inner.get("prompt")
        or inner.get("agent_internal_prompt")
        or inner.get("prompt_text")
...
def _looks_like_job_trigger(*, payload: dict[str, Any], inner: dict[str, Any]) -> bool:
    job_id = str(inner.get("job_id") or payload.get("job_id") or "").strip()
    user_id = str(inner.get("user_id") or payload.get("target") or "").strip()
    return bool(job_id and user_id)
