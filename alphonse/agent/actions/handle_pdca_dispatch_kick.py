...
class HandlePdcaDispatchKickAction(Action):
    ...
    def execute(self, context: dict) -> ActionResult:
        ...
        runtime_task = get_pdca_runtime().get_task(task_id)
        runtime_metadata = runtime_task.get("metadata") if isinstance(runtime_task, dict) and isinstance(runtime_task.get("metadata"), dict) else {}
        if bool(runtime_metadata.get("invoke_inflight")):
            _emit_skip(task_id=task_id, correlation_id=correlation_id, reason="invoke_inflight")
            return ActionResult(intention_key="NOOP", payload={}, urgency=None)

        latest = get_pdca_task(task_id)
        if not isinstance(latest, dict):
            _emit_skip(task_id=task_id, correlation_id=correlation_id, reason="task_not_found_after_refresh")
            return ActionResult(intention_key="NOOP", payload={}, urgency=None)

        latest_status = str(latest.get("status") or "").strip().lower()
        if latest_status in _TERMINAL_STATUSES:
            _emit_skip(task_id=task_id, correlation_id=correlation_id, reason="terminal_task_after_refresh")
            return ActionResult(intention_key="NOOP", payload={}, urgency=None)
        if latest_status not in _RUNNABLE_STATUSES:
            _emit_skip(task_id=task_id, correlation_id=correlation_id, reason="status_not_runnable_after_refresh")
            return ActionResult(intention_key="NOOP", payload={}, urgency=None)
        ...
        append_pdca_event(
            ...
            payload={"worker_id": "pdca-dispatcher", "scheduled_for": now.isoformat()},
