...
def test_event_fanout_filters_deduplicates_and_prunes_history() -> None:
    store = _store()
    first = store.create_event_automation(owner_user_id="alex", project_id="home", name="Water fern", prompt="Check the fern.", event_type="plant.soil_humidity", event_version="1", filters={"plant_id": "fern"})
    second = store.create_event_automation(owner_user_id="alex", project_id="home", name="Log humidity", prompt="Record the humidity.", event_type="plant.soil_humidity", event_version="1")
...
def test_daemon_enqueues_event_automation_with_structured_event_metadata() -> None:
    store = _store()
    store.create_event_automation(owner_user_id="local", project_id="home", name="Water fern", prompt="Check the fern.", event_type="plant.soil_humidity", event_version="1")
...
def test_automation_catalog_includes_existing_scheduled_tasks() -> None:
    ...
    schedules.create_task(owner_user_id="local", project_id="home", name="Morning", prompt="Good morning", schedule_kind="once", run_at="2030-01-01T00:00:00+00:00")
