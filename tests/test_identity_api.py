...
def test_identity_facade_resolves_canonical_user_and_service_mapping(monkeypatch, tmp_path) -> None:
    ...
    assert identity.get_user_by_principal_id("p-1")["user_id"] == "u-1"
...
def test_get_user_details_returns_canonical_user_id(monkeypatch, tmp_path) -> None:
    ...
    result = tool.execute(
        state={
            "actor_person_id": "u-cli",
            "incoming_user_id": "owner-cli",
            "channel_type": "cli",
            "channel_target": "cli",
            "chat_id": "cli:cli",
    ...
    assert output["resolved_user_id"] == "u-cli"
    assert output["service_key"] == "cli"
