...
def test_send_message_tool_executes_delivery() -> None:
    ...
    assert str(fake.request.target or "") == "8553589429"
...
def test_send_message_tool_accepts_canonical_user_id(tmp_path: Path, monkeypatch) -> None:
...
def test_send_message_tool_rejects_display_name(tmp_path: Path, monkeypatch) -> None:
    db_path = tmp_path / "nerve-db"
    monkeypatch.setenv("NERVE_DB_PATH", str(db_path))
    apply_schema(db_path)
    users_store.upsert_user(
...
def test_send_message_tool_rejects_current_conversation_alias_without_canonical_user_id() -> None:
    tool = SendMessageTool(_communication=_FakeCommunication())
    result = tool.execute(
        state={
            "channel_type": "cli",
            "channel_target": "cli",
            "conversation_key": "cli:cli",
            "actor_person_id": "owner-1",
            "incoming_user_id": "cli-admin",
            "incoming_user_name": "Alex",
            "correlation_id": "cid-cli-alias",
        },
        To="cli",
        Message="Hello back",
        Channel="cli",
...
def test_send_message_tool_accepts_canonical_user_id_for_cli_reply() -> None:
    db_path = Path("/tmp/alphonse-test-cli-user")
    apply_schema(db_path)
    users_store.upsert_user(
        {
            "user_id": "owner-1",
