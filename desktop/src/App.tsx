...
function ProjectsModal({ user, active, attention, onSelect, onSettings, onClose }: { user: string; active: Project | null; attention: ProjectAttention; onSelect: (project: Project) => void; onSettings: (project: ManagedProject) => void; onClose: () => void }) {
  const [projects, setProjects] = useState<ManagedProject[]>([]); const [filter, setFilter] = useState(""); const [showCreate, setShowCreate] = useState(false);
  ...
  const load = useCallback(async () => { try { const result = await daemonRequest<{ projects: ManagedProject[] }>("manageable_projects", { user, status: filter === "updates" ? "" : filter }); setProjects(result.projects); } catch (cause) { setNotice(cause instanceof Error ? cause.message : "Projects could not be loaded."); } }, [filter, user]);
  useEffect(() => { void load(); }, [load]);
  ...
  const visibleProjects = filter === "updates" ? projects.filter((item) => (attention[item.project_id]?.total || 0) > 0) : projects;
