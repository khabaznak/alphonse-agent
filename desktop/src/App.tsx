...
function SettingsModal({ user, initialTab, enterToSend, desktopStyle, onDesktopStyleChange, onEnterToSendChange, onTimezoneChange, onClose }: { user: string; initialTab: SettingsTab; enterToSend: boolean; desktopStyle: DesktopStyle; onDesktopStyleChange: (value: DesktopStyle) => void; onEnterToSendChange: (value: boolean) => void; onTimezoneChange: (value: string) => void; onClose: () => void }) {
  ...
  return <ModalFrame title="Settings" onClose={onClose}>
    ...
    {tab === "general" && <><label className="setting-row"><input type="checkbox" checked={enterToSend} onChange={(event) => onEnterToSendChange(event.target.checked)} /> Enter sends message</label><br/><label>Users root<input value={root} onChange={(event) => setRoot(event.target.value)} /></label><label>Timezone<input value={timezone} placeholder="America/Mexico_City" onChange={(event) => setTimezone(event.target.value)} /><small>Use an IANA timezone. New scheduled tasks use this unless a timezone is explicitly specified.</small></label><label className="setting-row"><input type="checkbox" checked={mirrorAutomationMessages} onChange={(event) => setMirrorAutomationMessages(event.target.checked)} /> Also send final reminders and scheduled-task messages to the recipient’s preferred channel</label><small>When enabled, Desktop delivery remains unchanged and Alphonse sends one additional copy to the recipient’s preferred configured channel.</small><button onClick={() => void save()}>Save</button><p>{notice}</p><section><h3>Conversation Memory</h3>{memory && <><label>Ledger limit (bytes)<input type="number" value={memory.max_ledger_bytes} onChange={(event) => setMemory({ ...memory, max_ledger_bytes: Number(event.target.value) })} /></label><label>Compaction limit (words)<input type="number" value={memory.compaction_summary_max_words} onChange={(event) => setMemory({ ...memory, compaction_summary_max_words: Number(event.target.value) })} /></label><button onClick={() => void saveMemory()}>Save Memory Settings</button></>}<p>{memoryNotice}</p></section></>}
