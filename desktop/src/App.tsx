...
function SettingsModal({ enterToSend, onEnterToSendChange, onClose }: { enterToSend: boolean; onEnterToSendChange: (value: boolean) => void; onClose: () => void }) {
  const [root, setRoot] = useState(""); const [notice, setNotice] = useState(""); const [tab, setTab] = useState<"general" | "tools">("general");
  ...
  return <ModalFrame title="Settings" onClose={onClose}><div><button onClick={() => setTab("general")} disabled={tab === "general"}>General</button><button onClick={() => setTab("tools")} disabled={tab === "tools"}>Tools</button></div>{tab === "general" ? <><label className="setting-row"><input type="checkbox" checked={enterToSend} onChange={(event) => onEnterToSendChange(event.target.checked)} /> Enter sends message</label><br/><label>Users root<input value={root} onChange={(event) => setRoot(event.target.value)} /></label><button onClick={() => void save()}>Save</button><p>{notice}</p><section><h3>Conversation Memory</h3>{memory && <><label>Ledger limit (bytes)<input type="number" value={memory.max_ledger_bytes} onChange={(event) => setMemory({ ...memory, max_ledger_bytes: Number(event.target.value) })} /></label><label>Compaction limit (words)<input type="number" value={memory.compaction_summary_max_words} onChange={(event) => setMemory({ ...memory, compaction_summary_max_words: Number(event.target.value) })} /></label><button onClick={() => void saveMemory()}>Save Memory Settings</button></>}<p>{memoryNotice}</p></section></> : <><section><h3>Web Tools</h3><p>Run SearXNG separately in Docker with JSON output enabled. Alphonse connects to it; it does not manage Docker.</p>{web && <><label className="setting-row"><input type="checkbox" checked={web.enabled} onChange={(event) => setWeb({ ...web, enabled: event.target.checked })} /> Enable Web Search and Fetch</label><label>SearXNG URL<input value={web.searxng_base_url} placeholder="http://127.0.0.1:8080" onChange={(event) => setWeb({ ...web, searxng_base_url: event.target.value })} /></label><label>Search timeout (seconds)<input type="number" value={web.search_timeout_seconds} onChange={(event) => setWeb({ ...web, search_timeout_seconds: Number(event.target.value) })} /></label><label>Fetch timeout (seconds)<input type="number" value={web.fetch_timeout_seconds} onChange={(event) => setWeb({ ...web, fetch_timeout_seconds: Number(event.target.value) })} /></label><label>Fetch text limit<input type="number" value={web.fetch_max_chars} onChange={(event) => setWeb({ ...web, fetch_max_chars: Number(event.target.value) })} /></label><button onClick={() => void saveWeb()}>Save Web Tools</button><button onClick={() => void verify("search")}>Verify SearXNG</button><button onClick={() => void verify("fetch")}>Verify Fetch</button></>}<p>{webNotice}</p></section><MediaToolsSettingsSection /></>}</ModalFrame>;
}

function MediaToolsSettingsSection() {
  const [settings, setSettings] = useState<MediaToolsSettings | null>(null); const [notice, setNotice] = useState(""); const [samples, setSamples] = useState<Record<string, string>>({ tts: "Alphonse text-to-speech verification.", stt: "", ocr: "" });
  ...
  const verify = async (kind: "tts" | "stt" | "ocr") => { try { const current = await daemonRequest<{ user: { user_id: string } | null }>("current_user"); if (!current.user) return; const result = await daemonRequest<{ settings: MediaToolsSettings; result: { exception?: { message?: string } } }>("verify_media_tools", { actor_user_id: current.user.user_id, kind, sample: samples[kind] }); setSettings(result.settings); setNotice(result.result.exception?.message || `${kind.toUpperCase()} verified.`); } catch (cause) { setNotice(cause instanceof Error ? cause.message : "Verification failed"); } };
  ...
  const verification = (kind: "tts" | "stt" | "ocr") => <p>Ready: {String(settings[kind].available)}. {settings[kind].verification.error || settings[kind].verification.preview || "Not yet verified."}</p>;
  return <section><h3>Local Media Tools</h3><p>Install models and runtimes separately, then save and verify each backend. These tools are not yet exposed to conversations.</p><h4>Qwen TTS</h4>{toggle("tts", "enabled", "Enable Qwen TTS")}{field("tts", "model_id", "Model ID or local path")}{field("tts", "device_map", "Device map")}{field("tts", "dtype", "Precision")}{field("tts", "language", "Language")}{field("tts", "speaker", "Speaker")}{field("tts", "instruct", "Voice instruction")}{field("tts", "attn_implementation", "Attention implementation")}{toggle("tts", "local_files_only", "Use local model files only")}{verification("tts")}<label>Test phrase<input value={samples.tts} onChange={(event) => setSamples({ ...samples, tts: event.target.value })} /></label><button onClick={() => void save("tts")}>Save TTS</button><button onClick={() => void verify("tts")}>Verify Qwen TTS</button>{settings.platform === "darwin" && <p>macOS say fallback: {settings.say_available ? "available" : "not found"}.</p>}<h4>Whisper STT</h4>{toggle("stt", "enabled", "Enable Whisper STT")}{field("stt", "executable_path", "Whisper executable (blank uses PATH)")}{field("stt", "model", "Model")}{field("stt", "default_language", "Default language")}{verification("stt")}<label>Audio sample path<input value={samples.stt} onChange={(event) => setSamples({ ...samples, stt: event.target.value })} /></label><button onClick={() => void save("stt")}>Save STT</button><button onClick={() => void verify("stt")}>Verify Whisper STT</button><h4>Ollama OCR</h4>{toggle("ocr", "enabled", "Enable OCR")}{field("ocr", "ollama_base_url", "Ollama URL")}{field("ocr", "model_id", "Vision model")}{field("ocr", "timeout_seconds", "Timeout seconds", "number")}{verification("ocr")}<label>Image sample path<input value={samples.ocr} onChange={(event) => setSamples({ ...samples, ocr: event.target.value })} /></label><button onClick={() => void save("ocr")}>Save OCR</button><button onClick={() => void verify("ocr")}>Verify OCR</button><p>{notice}</p></section>;
}
...
function ProjectsModal({ user, active, onSelect, onClose }: { user: string; active: Project | null; onSelect: (project: Project) => void; onClose: () => void }) {
  ...
  const [projects, setProjects] = useState<ManagedProject[]>([]); const [status, setStatus] = useState(""); const [selected, setSelected] = useState<ManagedProject | null>(null);
  const [name, setName] = useState(""); const [description, setDescription] = useState(""); const [visibility, setVisibility] = useState<"private" | "shared">("private"); const [rootPath, setRootPath] = useState(""); const [context, setContext] = useState(""); const [mode, setMode] = useState<"create" | "import">("create"); const [confirmation, setConfirmation] = useState(""); const [notice, setNotice] = useState("");
  const load = useCallback(async () => { try { const result = await daemonRequest<{ projects: ManagedProject[] }>("manageable_projects", { user, status }); setProjects(result.projects); setSelected((current) => current ? result.projects.find((item) => item.project_id === current.project_id) || null : null); } catch (cause) { setNotice(cause instanceof Error ? cause.message : "Projects could not be loaded."); } }, [status, user]);
  useEffect(() => { void load(); }, [load]);
  const choose = async (item: ManagedProject) => { setSelected(item); setName(item.name); setDescription(item.description); setVisibility(item.visibility); setRootPath(item.root_path); setConfirmation(""); try { const result = await daemonRequest<{ content: string }>("project_context", { user, project_id: item.project_id }); setContext(result.content); } catch (cause) { setNotice(cause instanceof Error ? cause.message : "Project context could not be loaded."); } };
  const save = async () => { if (!selected) return; try { await daemonRequest("update_project", { user, project_id: selected.project_id, name, description, visibility }); await daemonRequest("save_project_context", { user, project_id: selected.project_id, content: context }); setNotice("Project saved."); await load(); } catch (cause) { setNotice(cause instanceof Error ? cause.message : "Project could not be saved."); } };
  const createOrImport = async (event: FormEvent) => { event.preventDefault(); try { const method = mode === "import" ? "import_project" : "create_project"; const result = await daemonRequest<{ project: ManagedProject }>(method, { user, name, description, root_path: rootPath, visibility }); setNotice(mode === "import" ? "Project imported." : "Project created."); await load(); await choose(result.project); } catch (cause) { setNotice(cause instanceof Error ? cause.message : "Project could not be created."); } };
  const lifecycle = async (method: string) => { if (!selected) return; try { if (method === "delete_project") { await daemonRequest(method, { user, project_id: selected.project_id, confirmation }); setSelected(null); setContext(""); } else { await daemonRequest(method, { user, project_id: selected.project_id }); } setConfirmation(""); setNotice(method === "archive_project" ? "Project archived." : method === "restore_project" ? "Project restored." : "Project removed."); await load(); } catch (cause) { setNotice(cause instanceof Error ? cause.message : "Project lifecycle action failed."); } };
  return <ModalFrame title="Projects" onClose={onClose}>
    <div className="form-field"><label htmlFor="project-status-filter">Show</label><select id="project-status-filter" value={status} onChange={(event) => { setStatus(event.target.value); setSelected(null); }}><option value="">All projects</option><option value="active">Active</option><option value="archived">Archived</option></select></div>
    <div className="stack project-list">{projects.length ? projects.map((item) => <button className={item.project_id === selected?.project_id || item.project_id === active?.project_id ? "selected project-card" : "project-card"} key={item.project_id} onClick={() => void choose(item)}><span className="project-card-icon" aria-hidden="true">🗂️</span><span><strong>{item.name}</strong><small>👤 {item.owner?.display_name || item.owner_user_id}</small><small>{item.status} · {item.visibility} · created {dateLabel(item.created_at)}</small><small>Updated {dateLabel(item.updated_at)}</small></span></button>) : <p>No projects match this filter.</p>}</div>
    {selected ? <section className="project-detail"><div className="form-field"><label htmlFor="project-name">Name</label><input id="project-name" disabled={selected.status === "archived"} value={name} onChange={(event) => setName(event.target.value)} /></div><div className="form-field"><label htmlFor="project-description">Description</label><input id="project-description" disabled={selected.status === "archived"} value={description} onChange={(event) => setDescription(event.target.value)} /></div><div className="form-field"><label htmlFor="project-visibility">Visibility</label><select id="project-visibility" disabled={selected.status === "archived"} value={visibility} onChange={(event) => setVisibility(event.target.value as "private" | "shared")}><option value="private">Private</option><option value="shared">Shared</option></select></div><div className="form-field task-readonly"><label>Owner</label><output>👤 {selected.owner?.display_name || selected.owner_user_id}</output><small>{selected.owner_user_id}</small></div><div className="form-field task-readonly"><label>Project directory</label><output>{selected.root_path}</output><small>Created {dateLabel(selected.created_at)} · Updated {dateLabel(selected.updated_at)}</small></div><div className="form-field"><label htmlFor="project-context">Project context</label><textarea id="project-context" disabled={selected.status === "archived"} rows={8} value={context} onChange={(event) => setContext(event.target.value)} /></div>{selected.status === "active" && <div className="dialog-actions"><button onClick={() => onSelect(selected)}>Open project</button><button onClick={() => void save()}>Save changes</button><button onClick={() => void lifecycle("archive_project")}>Archive</button></div>}{selected.status === "archived" && <div className="dialog-actions"><button onClick={() => void lifecycle("restore_project")}>Restore</button></div>}<div className="form-field destructive-action"><label htmlFor="project-delete-confirmation">Type the project ID to remove it</label><input id="project-delete-confirmation" value={confirmation} placeholder={selected.project_id} onChange={(event) => setConfirmation(event.target.value)} /><button disabled={confirmation !== selected.project_id} onClick={() => void lifecycle("delete_project")}>Remove permanently</button></div></section> : <form className="project-create" onSubmit={createOrImport}><h3>{mode === "import" ? "Import existing folder" : "New project"}</h3><div className="dialog-actions"><button type="button" onClick={() => setMode("create")}>New</button><button type="button" onClick={() => setMode("import")}>Import</button></div><div className="form-field"><label htmlFor="new-project-name">Name</label><input id="new-project-name" value={name} onChange={(event) => setName(event.target.value)} required /></div><div className="form-field"><label htmlFor="new-project-description">Description</label><input id="new-project-description" value={description} onChange={(event) => setDescription(event.target.value)} /></div><div className="form-field"><label htmlFor="new-project-visibility">Visibility</label><select id="new-project-visibility" value={visibility} onChange={(event) => setVisibility(event.target.value as "private" | "shared")}><option value="private">Private</option><option value="shared">Shared</option></select></div><div className="form-field"><label htmlFor="new-project-path">{mode === "import" ? "Existing folder" : "Parent directory (optional)"}</label><input id="new-project-path" value={rootPath} onChange={(event) => setRootPath(event.target.value)} required={mode === "import"} /></div><button>{mode === "import" ? "Import project" : "Create project"}</button></form>}
    <p>{notice}</p>
  </ModalFrame>;
}
